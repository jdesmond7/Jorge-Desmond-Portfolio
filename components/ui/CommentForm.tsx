...
export function CommentForm({
  ...
  const resolvedHeading = heading ?? dict.comments.publishNew;
  const resolvedSubmitLabel = submitLabel ?? dict.comments.publish;
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error ?? dict.comments.errors.publishFailed);
        return;
      }
      ...
      setError(dict.comments.errors.publishFailed);
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <h3 className="text-[17px] font-semibold text-carbon">{resolvedHeading}</h3>
      ...
      <label className="block">
        <span className="sr-only">{dict.comments.bodyLabel}</span>
        <textarea
          value={body}
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          {submitting ? dict.comments.publishing : resolvedSubmitLabel}
        </button>
        {onCancel ? (
          <button
            type="button"
